...
#
# Search ROIs (POOL_ROI, CHEST_ROI) are saved via the Captures card's
# Search ROI button: drag the rect once, the bot loads it from JSON.
# Nothing is pasted into this file.
#
# Player animation needs no ROI: player_is_animating() tracks the red
# contrast-mode blob wherever it is.
#
...
bot = Bot(
    ...
    # A separate ten-recast limit catches missing animation even when
    # repeated clicks keep the no-click watchdog alive.
...
def preflight():
    if not _MISSING:
        if world().inventory is None and INV_FULL_REF is None:
            stop("Calibrate Inventory ROI or capture the inventory-full popup")
            return True
        return False
    log(
...
def walk_to_bank_when_full():
    ...
    if not chest:
        stop("Inventory full but bank chest was not found; check chest search area")
        return True
    reason = []
    if count_says_full:
        reason.append("count")
...
def recast_when_idle():
    ...
    if player_is_animating():
        ...
        _idle_ticks["n"] = 0
        return False
    if _recast_attempts["n"] >= 10:
        stop("Ten recasts without observed animation; check bait, target and calibration")
        return True
    _idle_ticks["n"] += 1
    if _idle_ticks["n"] < 3:
...
# idle=True keeps this from resetting the dry-tick watchdog, which
# is what lets auto_stop_dry_ticks actually catch a stuck bot.
# ─────────────────────────────────────────────────────────────────
